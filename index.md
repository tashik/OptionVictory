...
Если в комментариях к релизу не написано, что нужно обновить содержимое директории с программой целиком, для обновления используем сценарий инкрементной установки (обновляем только изменившиеся файлы). 
...
### Версия от 23 марта 2025 г (АКТУАЛЬНАЯ)
[ovic-2.5.2](https://github.com/tashik/OptionVictory/archive/refs/tags/ovic-2.5.2.zip) Переход Мосбиржи на единую торговую сессию (исправление времени экспирации). Исправление работы с Moex Option Calculator (TLS)

Для инкрементной установки с версии 2.3.8 или более поздней просто заменяем OptionFVV.exe и OptionFVV.pdb на те, что в релизе.

### Версия от 01 апреля 2025 г
[ovic-2.5](https://github.com/tashik/OptionVictory/archive/refs/tags/ovic-2.5.zip) Поддержка расчета гарантийного обеспечения с использованием API опционного калькулятора Мосбиржи. Внесены мелкие исправления в интерфейс в раздел "Данные". Исправлена проблема отрисовки диаграммы при нулевом PNL стратегии.

Для инкрементной установки с версии 2.3.8 или более поздней просто заменяем OptionFVV.exe и OptionFVV.pdb на те, что в релизе.

### Версия от 27 октября 2023 г
[ovic-2.4.2](https://github.com/tashik/OptionVictory/archive/refs/tags/ovic-2.4.2.zip) Исправлена ошибка для режима абсолютных настроект "что если". Внесены мелкие исправления в интерфейс в раздел "Данные". Для релиза достаточно инкрементной установки с версии 2.3.8 или 2.4.*

### Версия от 30 июня 2023 г
[ovic-2.4.1](https://github.com/tashik/OptionVictory/archive/refs/tags/ovic-2.4.1.zip) Исправлена ошибка на вкладке Улыбка, из-за которой улыбка не отрисовывалась для опционов на некоторые базовые активы. Учтены другие коды классов для недельных опционов у брокера ITI Capital. Для релиза достаточно инкрементной установки с версии 2.3.8 или 2.4.

Для инкрементной установки просто заменяем OptionFVV.exe и OptionFVV.pdb на те, что в релизе.

### Версия от 10 июня 2023 г
[ovic-2.4](https://github.com/tashik/OptionVictory/archive/refs/tags/ovic-2.4.zip) Поддержка разной точности цен опционов и базовых активов, позволяющая правильно учитывать и моделировать позиции в опционах на юань и природный газ, а также акционы типа ВТБ, у которых после запятой в ценах более чем два знака. **В Таблицу текущих торгов потребуется добавить колонку ТОЧНОСТЬ после колонок Код класса и Лот** (внимание! не Точность к-ва, а просто Точность). Для релиза достаточно инкрементной установки с версии 2.3.8

Для инкрементной установки с версии 2.3.8 просто заменяем OptionFVV.exe и OptionFVV.pdb на те, что в релизе.

### Версия от 14 января 2023 г
[ovic-2.3.8](https://github.com/tashik/OptionVictory/archive/refs/tags/ovic-2.3.8.zip) Импорт позиции из таблицы Позиций по клиентским счетам. Поддержка учета хеджирования премиальных опционов на акции фьючерсом на акции

Для инкрементной установки с версии 2.3.6 или новее просто заменяем OptionFVV.exe и OptionFVV.pdb на те, что в релизе.

**ВНИМАНИЕ! ПРОИЗОШЛИ ИЗМЕНЕНИЯ В НАСТРОЙКАХ ТАБЛИЦЫ ТЕКУЩИХ ТОРГОВ!** Перед обновлением нужно добавить последним столбец **ЛОТ**, после столбца **КОД КЛАССА** (а если его нет - то и его добавить после столбца **ШАГ ЦЕНЫ**).

**КАК ИМПОРТИРОВАТЬ ПОЗИЦИЮ ИЗ QUIK?**
1.В Quik потребуется создать таблицу Позиции по клиентским счетам со следущим списком колонок (порядок важен): Счет, Код инструмента, Тек.чист.поз, Эффективная цена поз
2. После создания правой кнопкой вызываем контекстное меню этой таблицы и выбираем **Копировать все**
3. После копирования если в таблице только позиции по создаваемой нами стратегии, просто вставляем содержимое буфера обмена в большое текстовое поле на левой панели вкладки Данные в OptionVictory и нажимаем Импортировать (предварительно придумав имя стратегии и введя его в соответствующее поле). Если же в таблице позиций были позиции на разные базовые активы, потребуется сначала отредактировать содержимое буфера обмена в любом текстовом редакторе, удалив нерелеватные позиции.
4. После нажатия на кнопку Импортировать внизу жмем Обновить - и стратегия с заполненными позициями появится в списке

Как вести позицию в премиальных опционах, хеджируемую фьючерсом на ации, и осуществлять реал-тайм учет изменений в ней, я сниму видео и приложу ссылку.

### Версия от 04-05 декабря 2022 г
